...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (searchParams.get('kicked') === '1') {
      setTimeout(() => showToast('You were removed by the admin.', 'error'), 500);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/team/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ teamCode: teamCode.trim(), password: password.trim() }),
      ...
      <div style={{ width: '100%', maxWidth: 500 }}>
        <div className="white-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
            <div style={{
              width: 48, height: 48, background: 'var(--primary-gradient)', borderRadius: 12,
              color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center'
            }}>
              <span className="material-symbols-rounded">co_present</span>
            </div>
            <div>
              <h2 style={{ margin: 0, fontWeight: 800, fontSize: '1.5rem' }}>Welcome, Team!</h2>
              <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.9rem' }}>Enter your credentials to join</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              {loading
                ...
                : <>Login <span className="material-symbols-rounded" style={{ fontSize: 18 }}>bolt</span></>
              }
            </button>
          </form>

          <div style={{
            textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '1.5rem',
            ...
            Use credentials provided by your organizer
          </div>
        </div>
      </div>
    </div>
